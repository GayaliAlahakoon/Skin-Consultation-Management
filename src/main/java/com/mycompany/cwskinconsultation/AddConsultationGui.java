...
public class AddConsultationGui extends JFrame implements Serializable {
    ...
    public void checkAvailability(){
        ...
        if (userEnteredDateOfBirth.isEmpty()) {
            ...
            try
            {
                Date dob = sDateFormat.parse(userEnteredDateOfBirth);
            }
            /* Date format is invalid */
            catch (ParseException e)
            {
                infoBox(userEnteredDateOfBirth + "is not a valid date format", "warning");
            }
        }
        ...
        if (userEnteredConsultDate.isEmpty()) {
            ...
            try
            {
                Date dob = sDateFormat.parse(userEnteredConsultDate);
            }
            /* Date format is invalid */
            catch (ParseException e)
            {
                infoBox(userEnteredConsultDate + "is not a valid date format", "warning");
            }
        }
        ...
        if (Consultation.consultations.size() == 0){
            ...
            Doctor.temporaryDoctors.addAll(Doctor.doctors);
    ...
    public void doctorAvailability(Doctor doctor, String consultationSelectedDate, String selectedTimeSlot, Patient patient){
        ...
        for (Consultation consultation : Consultation.consultations){
            if (consultation.getDoctor().getMedicalLicenceNo() == doctor.getMedicalLicenceNo()){
        ...
        if (selectedDocHasConsultation){
            for (Consultation consultation : Consultation.consultations){
                if (consultation.getDate().equals(consultationSelectedDate) &&
                        ...
                        consultation.getDoctor().getMedicalLicenceNo() == doctor.getMedicalLicenceNo()){
        ...
        if (selectedDocHasConsultationSelectedDateTime){
            Doctor.temporaryDoctors.remove(doctor);
            if (Doctor.temporaryDoctors.size()==0){
                infoBox("No doctors available at selected time and date", "warning");
            }else{
                int randomDoctorIndex = random.nextInt(Doctor.temporaryDoctors.size());
                randomDoctor = Doctor.temporaryDoctors.get(randomDoctorIndex);
